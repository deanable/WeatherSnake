...
### Added
- **Template picker for Infogram**: the *Infogram Token* button now fetches the account's project list (`getProjectList`) after the token is entered and shows a picker dialog — choose the template project instead of hunting down its UUID. Manual ID entry remains available as a fallback (list fetch failure, empty account, or the picker's *Enter ID manually* button).
