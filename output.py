...
def generate_visualizations(df: pd.DataFrame, city: str, period: str, units: str, monthly: bool):
    """Generates and saves temperature and rainfall graphs as a PNG file."""
    fig = create_visualization_figure(df, city, period, units, monthly)
    filename = f"weather_plot_{city.replace(' ', '_')}_{period.replace(' ', '_')}.png"
    fig.savefig(filename)
    plt.close(fig)
    print(f"Saved visualization to {filename}")

def create_visualization_figure(df: pd.DataFrame, city: str, period: str, units: str, monthly: bool):
